...
const TAG_ICONS = {
  'React': '⚛', 'Node.js': '🟢', 'JavaScript': 'JS', 'TypeScript': 'TS',
  'Vite': '⚡', 'vite': '⚡', 'Scss': '🎨', 'css': '🖌', 'Router': '🔗',
  'Swiper': '↔', 'ContextAPI': '🔄', 'LocalStorage': '💾',
  'Open Source API': '🌐', 'OpenWeatherAPI': '🌤', 'Axios': '📡',
}
...
const ProjectCard = ({ title, tags, status, desc, codeUrl, thumbImage, thumbVariant }) => {
  ...
  const isLinked = codeUrl && codeUrl !== '#'

  return (
    <article
      className={`${styles.card} ${isLinked ? styles.linked : ''}`}
      onClick={handleCardClick}
      role={isLinked ? 'link' : undefined}
      tabIndex={isLinked ? 0 : undefined}
      onKeyDown={isLinked ? (e) => e.key === 'Enter' && handleCardClick() : undefined}
      aria-label={isLinked ? `${title} GitHub 보기` : undefined}
    >
      {/* 썸네일 */}
